$(document).ready(function() {
    ...
    var table = $('#advance-11-new').DataTable({
        "columnDefs": [
            { "visible": false, "targets": 2 }
        ],
        "order": [[2, 'asc']],
        ...
        "drawCallback": function (settings) {
            ...
            api.column(2, { page: 'current' }).data().each(function (group, i) {
                if (last !== group) {
                    $(rows).eq(i).before(
                        '<tr class="group"><td colspan="5" style="background-color:	#E5E4E2; font-weight: bold;">' + group + '</td></tr>'
                    );
                    last = group;
                }
            });
    ...
    $('#advance-11-new tbody').on('click', 'tr.group', function () {
        var currentOrder = table.order()[0];
        if (currentOrder[0] === 2 && currentOrder[1] === 'asc') {
            table.order([2, 'desc']).draw();
        }
